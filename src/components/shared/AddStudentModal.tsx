...
const AddStudentModal: React.FC<AddTeacherModalProps> = ({
  isOpen,
  onClose,
}) => {
  ...
  const handleCopyLink = () => {
    const link = `http://localhost:5173/student-sign-up/?schoolId=${schoolId}`;
    navigator.clipboard
      .writeText(link)
      .then(() => {
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div>
          <p className="bg-slate-100 border-t-0 border-l-0 border-r-0 border-b border-[#0BC279] p-3">
            {`http://localhost:5173/student-sign-up/?schoolId=${schoolId}`}
          </p>
        </div>
